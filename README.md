...
核心设计：每轮结束后，用**固定占位符**原地替换该轮工具结果，而非直接删除。真正省 Token 的是「清除」这件事本身——历史不再携带全部工具结果；占位符保住 `tool_call`/`result` 配对，并给模型留下取回索引。代价是每轮首个请求会在上一轮第一个工具结果处断开、按全价重算一次上一轮尾部（实测净省 3%~20%，随轮数增长）。
